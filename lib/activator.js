		...
		async.waterfall([
			...
			function(res,cb){
				if (!res) {
					...
					code = jwt.sign({sub:email,purpose:"activation",exp:((new Date().getTime())/1000 + resetExpire*60), jti: mongodb.ObjectId(), _id: mongodb.ObjectId()},signkey,{algorithm:"HS256"});
					token = code;
					if (!email) {
						cb("missingemail");
					} else if (!id) {
		...
		function (res,cb) {
			if (!res) {
				...
				try {
					...
					model.activate(idProperty?getObjectProperty(res, idProperty):id, code, cb);
				} catch (e) {
					cb("invalidcode");
				}
			}
	...
	async.waterfall([
		...
		function (res,cb) {
			if (!res || res.length < 1) {
				...
				reset_code = jwt.sign({sub:email,purpose:"resetpassword",exp:((new Date().getTime())/1000 + resetExpire*60), jti: mongodb.ObjectId(), _id: mongodb.ObjectId()},signkey,{algorithm:"HS256"});
				token = reset_code;
